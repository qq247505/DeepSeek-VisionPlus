...
- 🎨 **模型页内嵌卡片**：设置 → 模型页里的 DeepSeek VisionPlus 卡片——添加平台、配置密钥、编辑模型目录（ID/名称/上下文窗口/最大输出，K/M 单位），保存实时生效；
- 🔌 **输入框图片按钮**：选中视觉变体后出现，选图 / 多选 / 拖拽即发；
- 🧪 **一键测试**：DeepSeek 与每个视觉平台都有测试按钮——先校验参数（密钥/地址/模型 ID/官方上限），再按官方对接方式真实请求，结果气泡提示 3 秒消失；
...
> 数值来自官方文档模型概览与实测 API 限制；自定义平台按对方官方文档填写。
